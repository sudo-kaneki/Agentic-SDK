...
def calculate_energy_cost(kwh: float, rate_per_kwh: float = 0.12) -> dict:
    ...
    return {"kwh_consumed": kwh, "rate_per_kwh": rate_per_kwh, "total_cost": round(total_cost, 2)}
...
class StatefulConversationManager:
    ...
    async def handle_conversation_turn(
        self, session_id: str, user_id: str, user_input: str, agent_name: str = "EnergyConsultant"
    ) -> dict:
        """
...
"""
            ...
            return {"response": agent_response, "session_info": session_info, "success": True}
...
async def demo_stateful_conversation():
    ...
    for i, user_input in enumerate(conversation_turns, 1):
        ...
        result = await manager.handle_conversation_turn(session_id, user_id, user_input)
        ...
        if result["success"]:
            ...
            if session_info.get("persistent"):
                print(
                    f"📊 Session: {session_info['total_messages']} messages, "
