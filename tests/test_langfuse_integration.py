...
class TestLangfuseIntegration:
    ...
    def app_with_langfuse(self, mock_langfuse_client):
        ...
        app = EnergyAIApplication(
            title="Test App with Langfuse", langfuse_monitoring_client=mock_langfuse_client
        )
        return app
    ...
    @pytest.mark.asyncio
    async def test_chat_request_with_session_context_monitoring(
        self, app_with_langfuse, mock_agent
    ):
    ...
    async def test_chat_request_error_handling_with_langfuse(self, app_with_langfuse, mock_agent):
        ...
        request = ChatRequest(message="Error test", agent_id="TestAgent")
        ...
        with patch("energyai_sdk.application.agent_registry") as mock_registry:
            ...
            error_update = next(
                (call for call in update_trace_calls if call[1]["level"] == "ERROR"), None
            )
    ...
    async def test_chat_request_without_langfuse_client(self):
        ...
        request = ChatRequest(message="Hello without monitoring", agent_id="TestAgent")
    ...
    async def test_context_span_error_handling(self, app_with_langfuse, mock_agent):
        ...
        request = ChatRequest(
            message="Context error test", agent_id="TestAgent", session_id="error_session"
        )
        ...
        with patch("energyai_sdk.application.agent_registry") as mock_registry:
            ...
            error_call = next(
                (call for call in end_span_calls if call[1]["level"] == "ERROR"), None
            )
            assert error_call is not None
    ...
    def test_langfuse_configuration_variations(self):
        ...
        test_cases = [
            ...
            },
            {
                ...
                "params": {"langfuse_environment": "production", "debug": False},
            },
            {
                "description": "Development configuration",
                "params": {"langfuse_environment": "development", "debug": True},
            },
        ]
